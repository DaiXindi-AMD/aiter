...
def _rmsnorm_bwd_kernel_large_m_small_n(
    ...
    # M=b*s*heads, N=head_dim). Each persistent program grid-strides over
    # BLOCK_M x BLOCK_N tiles, accumulating one private dgamma partial. The
    # existing _rmsnorm_bwd_dg_reduce kernel finishes the cross-program sum.
    pid_m = tl.program_id(0)
    n_off = tl.arange(0, BLOCK_N)
    mask_n = n_off < N
    ...
    for block_idx in range(pid_m, tl.cdiv(M, BLOCK_M), NUM_PRGMS):
        m_off = block_idx * BLOCK_M + tl.arange(0, BLOCK_M)
        mask_m = m_off < M
        mask = mask_m[:, None] & mask_n[None, :]
        ...
        norm_factor = tl.load(
            rsigma_ptr + m_off, mask=mask_m, other=0.0
        ).to(tl.float32)

        # Per-row grad_sum over N (masked-out columns already contribute zero).
        grad_sum = tl.sum(grad_output * x * g[None, :], axis=1)
        coeff = (norm_factor * norm_factor * norm_factor) * (grad_sum / N)
        grad_input = (
            grad_output * norm_factor[:, None] * g[None, :] - coeff[:, None] * x
        )
        tl.store(
        ...
        dg_acc += tl.sum(grad_output * x * norm_factor[:, None], axis=0)

    tl.store(dg_ptr + pid_m * N + n_off, dg_acc, mask=mask_n)
